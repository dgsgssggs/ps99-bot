...
class Database:
        ...
        async with aiosqlite.connect(self.path) as db:
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS affiliates (
                    ...
                    referrer_id   TEXT,
                    gems_generated INTEGER DEFAULT 0
                )
            """)
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS events (
                    ...
                    record_value REAL DEFAULT 0,
                    holder_id    TEXT DEFAULT '',
                    holder_name  TEXT DEFAULT ''
                )
            """)

            # Coinflip streak table
            await db.execute("""
                CREATE TABLE IF NOT EXISTS coinflip_streak (
                    discord_id TEXT PRIMARY KEY,
    ...
    async def set_affiliate(self, referred_id: str, referrer_id: str) -> bool:
        """Link referred_id to referrer_id. Returns False if already set."""
        async with aiosqlite.connect(self.path) as db:
            cursor = await db.execute(
                "SELECT referrer_id FROM affiliates WHERE referred_id = ?", (referred_id,)
            )
            if await cursor.fetchone():
                return False
            await db.execute(
                "INSERT OR IGNORE INTO affiliates (referred_id, referrer_id, gems_generated) VALUES (?, ?, 0)",
                (referred_id, referrer_id)
            )
            await db.commit()
            return True

    async def get_referrer(self, discord_id: str) -> str | None:
        """Get the referrer of a user."""
        async with aiosqlite.connect(self.path) as db:
            cursor = await db.execute(
                "SELECT referrer_id FROM affiliates WHERE referred_id = ?", (discord_id,)
            )
            row = await cursor.fetchone()
            return row[0] if row else None

    async def add_affiliate_earnings(self, referrer_id: str, referred_id: str, amount: int):
        """Add affiliate earnings to referrer and log them."""
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                "UPDATE affiliates SET gems_generated = gems_generated + ? WHERE referred_id = ?",
                (amount, referred_id)
            )
            await db.execute(
                "UPDATE users SET balance = balance + ? WHERE discord_id = ?",
    ...
    async def get_affiliates(self, referrer_id: str):
        """Get all referrals and their generated earnings for a referrer."""
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
                """SELECT a.referred_id, a.gems_generated, u.roblox_name
                   FROM affiliates a
                   LEFT JOIN users u ON u.discord_id = a.referred_id
                   WHERE a.referrer_id = ?
                   ORDER BY a.gems_generated DESC""",
                (referrer_id,)
            )
            return await cursor.fetchall()

    async def get_total_affiliate_generated(self, referrer_id: str) -> int:
        """Total gems generated by all referrals of this user."""
        async with aiosqlite.connect(self.path) as db:
            cursor = await db.execute(
                "SELECT COALESCE(SUM(gems_generated), 0) FROM affiliates WHERE referrer_id = ?",
                (referrer_id,)
            )
            row = await cursor.fetchone()
            return row[0] if row else 0
    ...
    async def get_event(self, event_type: str):
        """Get event state."""
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
    ...
    async def set_event_active(self, event_type: str, active: bool):
        """Enable or disable an event."""
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                """INSERT INTO events (type, active, record_value, holder_id, holder_name)
                   VALUES (?, ?, 0, '', '')
                   ON CONFLICT(type) DO UPDATE SET active = ?""",
                (event_type, 1 if active else 0, 1 if active else 0)
            )
            if active:  # Reset record when starting
                await db.execute(
                    "UPDATE events SET record_value = 0, holder_id = '', holder_name = '' WHERE type = ?",
                    (event_type,)
                )
            await db.commit()

    async def update_event_record(self, event_type: str, value: float, holder_id: str, holder_name: str) -> bool:
        """Update event record if value beats current. Returns True if new record."""
        async with aiosqlite.connect(self.path) as db:
            cursor = await db.execute(
                "SELECT active, record_value FROM events WHERE type = ?", (event_type,)
            )
            row = await cursor.fetchone()
            ...
            if value > row[1]:
                await db.execute(
                    "UPDATE events SET record_value = ?, holder_id = ?, holder_name = ? WHERE type = ?",
                    (value, holder_id, holder_name, event_type)
                )
                await db.commit()
                return True
            return False

    async def get_coinflip_streak(self, discord_id: str) -> int:
        async with aiosqlite.connect(self.path) as db:
            cursor = await db.execute(
                "SELECT streak FROM coinflip_streak WHERE discord_id = ?", (discord_id,)
            )
            row = await cursor.fetchone()
            return row[0] if row else 0

    async def set_coinflip_streak(self, discord_id: str, streak: int):
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                """INSERT INTO coinflip_streak (discord_id, streak) VALUES (?, ?)
                   ON CONFLICT(discord_id) DO UPDATE SET streak = ?""",
                (discord_id, streak, streak)
            )
            await db.commit()
